var sConnect = (function () {
  ...
  return function (co) {
    ...
    var urlUse =
      location.protocol + "//" + h + "/cake4/rd_cake/radaccts/get_usage.json";
    var urlUam = "uam.php";
    ...
    var urlSocialBase =
      ...
      "//" +
      h +
      "/cake4/rd_cake/third-party-auths/index.json";

    //To pull the username and password associated with this ID + typ
    var urlSocialInfoFor =
      location.protocol +
      "//" +
      h +
      "/cake4/rd_cake/third-party-auths/info-for.json";
    ...
    var urlAdd =
      ...
      "//" +
      h +
      "/cake4/rd_cake/register-users/new-permanent-user.json";
    var urlLostPw =
      location.protocol +
      "//" +
      h +
      "/cake4/rd_cake/register-users/lost-password.json";
    var urlOtp =
      location.protocol +
      "//" +
      h +
      "/cake4/rd_cake/register-users/otp-submit.json";
    var urlReqOtp =
      location.protocol +
      "//" +
      h +
      "/cake4/rd_cake/register-users/otp-request.json";
    var urlOtpCtc =
      location.protocol +
      "//" +
      h +
      "/cake4/rd_cake/data-collectors/otp-submit.json";
    var urlReqOtpCtc =
      location.protocol +
      "//" +
      h +
      "/cake4/rd_cake/data-collectors/otp-request.json";
    ...
    var extractHotspotError = function (j, defaultMsg) {
      ...
      if (
        ...
        typeof j.message !== "undefined" &&
        j.message !== null &&
        j.message !== ""
      ) {
        msg = j.message;
      } else if (
        typeof j.error !== "undefined" &&
        j.error !== null &&
        j.error !== ""
      ) {
        msg = j.error;
      } else if (
        typeof j.reason !== "undefined" &&
        j.reason !== null &&
        j.reason !== ""
      ) {
        msg = j.reason;
      ...
      if (
        msg.includes("Sorry... you have used your") &&
        msg.includes("data allowance")
      ) {
        if (msg.includes("all your")) {
          return i18n("Sorry... you have used all your data allowance");
        } else {
          return i18n("Sorry... you have used your data allowance");
        }
      }
      if (msg === "Maximum usage exceeded") {
        return i18n("Maximum usage exceeded");
      }
      if (msg.startsWith("Account activate on")) {
        return (
          i18n("Account activate on") +
          msg.substring("Account activate on".length)
        );
      }
      if (msg === "Denied access by rlm_perl function") {
        return i18n("Denied access by rlm_perl function");
      }
      if (msg === "Required Request Attributes Missing") {
        return i18n("Required Request Attributes Missing");
      }
      if (msg === "Missing SSID in Called-Station-Id") {
        return i18n("Missing SSID in Called-Station-Id");
      }
      if (msg === "No PPSK Match Found") {
        return i18n("No PPSK Match Found");
      }
      if (msg.startsWith("Missing Cleartext Password For")) {
        return (
          i18n("Missing Cleartext Password For") +
          msg.substring("Missing Cleartext Password For".length)
        );
      }
      if (msg.startsWith("Simultaneous connections limited to")) {
        return (
          i18n("Simultaneous connections limited to") +
          msg.substring("Simultaneous connections limited to".length)
        );
      }
      if (msg === "Your account has expired") {
        return i18n("Your account has expired");
      }
      if (msg === "Sorry.. the router has reached its monthly data limit") {
        return i18n("Sorry.. the router has reached its monthly data limit");
      }
      if (msg.includes("Not Available To Use On")) {
        return (
          i18n("Not Available To Use On") +
          msg.substring("Not Available To Use On".length)
        );
      }
    ...
    var refreshCounter = function () {
      ...
      counter = setInterval(function () {
        ...
        if (false) {
          //We remove ourself gracefully FIXME
          clearInterval(counter);
          counter = undefined;
          timeUntilStatus = refreshInterval;
        } else {
          $("#status_refresh").text(timeUntilStatus);
          if (timeUntilStatus == 0) {
            //Each time we reach null we refresh the screens
    ...
    var rdUsageRefresh = function () {
      ...
      $.getJSON(urlUse, { username: un, mac: mac }, function (j) {
        fDebug(j);
        ...
        if (j.data.time_cap == null) {
          ...
          $("#pbTime")
            .css("width", pers_time_used + "%")
            .text(pers_time_used + "%");
          $("#divTime").html(
            "<strong>" +
              i18n("sUsed") +
              " </strong>" +
              time(j.data.time_used) +
              "<strong> " +
              i18n("sAvailable") +
              " </strong>" +
              time(time_avail),
        ...
        if (j.data.data_cap == null) {
          ...
          $("#pbData")
            .css("width", pers_data_used + "%")
            .text(pers_data_used + "%");
          $("#divData").html(
            "<strong>" +
              i18n("sUsed") +
              " </strong>" +
              bytes(j.data.data_used) +
              "<strong> " +
              i18n("sAvailable") +
              " </strong>" +
              bytes(data_avail),
    ...
    var onBtnClickToConnectClickPre = function (event) {
      ...
      if (cDynamicData.settings.click_to_connect.cust_info_check == false) {
        ...
        var email_check =
          ...
          "/cake4/rd_cake/data-collectors/mac-check.json";
        ...
        $.ajax({
          ...
          dataType: "json",
          timeout: 3000,
          data: { mac: mac_address, nasid: nasid, i18n: s_i18n },
        })
          .done(function (j) {
            if (j.success == true) {
    ...
    var populateCustInfo = function () {
      ...
      if (cDynamicData.settings.click_to_connect.ci_first_name) {
        ...
        var $first_name =
          `
                    <div class="mb-3 ` +
          $first_name_req_class +
          `">
                        <input type="text" placeholder="` +
          i18n("sFirstName") +
          `" class="form-control" id="ciFirstName" name="first_name" ` +
          $first_name_req_attr +
          `>
                        <div class="invalid-feedback">
                            ` +
          i18n("sPlease_supply_a_valid") +
          " " +
          i18n("sFirstName") +
          `
                        </div>
      ...
      if (cDynamicData.settings.click_to_connect.ci_last_name) {
        ...
        var $last_name =
          ...
          `">
                        <input type="text" placeholder="` +
          i18n("sSurname") +
          `" class="form-control" id="ciSurname" name="last_name" ` +
          $last_name_req_attr +
          `>
                        <div class="invalid-feedback">
                             ` +
      ...
      if (cDynamicData.settings.click_to_connect.ci_email) {
        ...
        var $email =
          ...
          i18n("sEmail") +
          `" class="form-control" id="ciEmail" name="email" ` +
          $email_req_attr +
          `>
                        <div class="invalid-feedback">
                            ` +
        ...
        if (cDynamicData.settings.click_to_connect.ci_email_opt_in) {
          var $email_opt_in =
            ...
            cDynamicData.settings.click_to_connect.ci_email_opt_in_txt +
            `</label>
                            </div>     
                        </div>`;
      ...
      if (cDynamicData.settings.click_to_connect.ci_birthday) {
        ...
        var $birthday =
          ...
          $birthday_req_class +
          `">
                        <label for="ciBirthday">` +
          i18n("sBirthday") +
          `</label>
                        <input id="ciBirthday" class="form-control" type="date" name="birthday"  ` +
          $birthday_req_attr +
          `/>
                        <div class="invalid-feedback">
                            ` +
      ...
      if (cDynamicData.settings.click_to_connect.ci_company) {
        ...
        var $company =
          ...
          `">
                        <input type="text" placeholder="` +
          i18n("sCompany") +
          `" class="form-control" id="ciCompany" name="company" ` +
          $company_req_attr +
          `>
                        <div class="invalid-feedback">
                            ` +
      ...
      if (cDynamicData.settings.click_to_connect.ci_address) {
        ...
        var $address =
          ...
          i18n("sAddress") +
          `" class="form-control" id="ciAddress" rows="3" name="address" ` +
          $address_req_attr +
          `></textarea>
                        <div class="invalid-feedback">
                            ` +
      ...
      if (cDynamicData.settings.click_to_connect.ci_city) {
        ...
        var $city =
          ...
          `">
                        <input type="text" placeholder="` +
          i18n("sCity") +
          `" class="form-control" id="ciCity" name="city" ` +
          $city_req_attr +
          `>
                        <div class="invalid-feedback">
                            ` +
      ...
      if (cDynamicData.settings.click_to_connect.ci_country) {
        ...
        var $country =
          ...
          `">
                        <input type="text" placeholder="` +
          i18n("sCountry") +
          `" class="form-control" id="ciCountry" name="country" ` +
          $country_req_attr +
          `>
                        <div class="invalid-feedback">
                             ` +
      ...
      if (cDynamicData.settings.click_to_connect.ci_phone) {
        ...
        var $phone =
          ...
          `">
                        <input type="text" placeholder="` +
          i18n("sPhone") +
          `" class="form-control" id="ciPhone" minlength="8" pattern="^[0-9]*$" name="phone" ` +
          $phone_req_attr +
          `>
                        <div class="invalid-feedback">
                            ` +
            ...
            `
                        <div class="mb-3">
                            <div class="form-check form-switch">
                                ...
                                <label class="form-check-label" for="flexSwitchCheckChecked">` +
            cDynamicData.settings.click_to_connect.ci_phone_opt_in_txt +
            `</label>
                            </div>     
                        </div>`;
      ...
      if (cDynamicData.settings.click_to_connect.ci_room) {
        ...
        var $room =
          ...
          `">
                        <input type="text" placeholder="` +
          i18n("sRoom") +
          `" class="form-control" id="ciRoom" name="room" ` +
          $room_req_attr +
          `>
                        <div class="invalid-feedback">
                            ` +
      ...
      if (cDynamicData.settings.click_to_connect.ci_custom1) {
        ...
        var $custom1 =
          ...
          `">
                        <input type="text" placeholder="` +
          cDynamicData.settings.click_to_connect.ci_custom1_txt +
          `" class="form-control" id="ciCustom1" name="custom1" ` +
          $custom1_req_attr +
          `>
                        <div class="invalid-feedback">
                            ` +
      ...
      if (cDynamicData.settings.click_to_connect.ci_custom2) {
        ...
        var $custom2 =
          ...
          `">
                        <input type="text" placeholder="` +
          cDynamicData.settings.click_to_connect.ci_custom2_txt +
          `" class="form-control" id="ciCustom2" name="custom2" ` +
          $custom2_req_attr +
          `>
                        <div class="invalid-feedback">
                            ` +
      ...
      if (cDynamicData.settings.click_to_connect.ci_custom3) {
        ...
        var $custom3 =
          ...
          `">
                        <input type="text" placeholder="` +
          cDynamicData.settings.click_to_connect.ci_custom3_txt +
          `" class="form-control" id="ciCustom3" name="custom3" ` +
          $custom3_req_attr +
          `>
                        <div class="invalid-feedback">
                            ` +
    ...
    var onBtnCustInfoClick = function (event) {
      ...
      if (!form.checkValidity()) {
        ...
        var add_mac =
          ...
          "//" +
          document.location.hostname +
          "/cake4/rd_cake/data-collectors/add-mac.json";
        ...
        if ($custom1) {
          ...
          if ($custom1_placeholder == "DN") {
            if (
              numbers_only.test($($custom1).val()) == false ||
              $($custom1).val().length < 7
            ) {
        ...
        if ($custom2) {
          ...
          if ($custom2_placeholder == "DN") {
            if (
              numbers_only.test($($custom2).val()) == false ||
              $($custom2).val().length < 7
            ) {
        ...
        if ($custom3) {
          ...
          if ($custom3_placeholder == "DN") {
            if (
              numbers_only.test($($custom3).val()) == false ||
              $($custom3).val().length < 7
            ) {
    ...
    var socialTempLoginResultsMt = function (j) {
      ...
      if (j.logged_in == "no") {
        fShowError(
          extractHotspotError(
            j,
            i18n("sAuthentication_failure_please_try_again"),
          ),
        );
      } else {
        ...
        $.each(notRequired, function (index, value) {
          //FIXME adapt the notRequired list for MT
    ...
    var checkSocialLoginReturnMt = function () {
      if (
        getParameterByName("sl_type") != "" && //e.g. user or voucher
        ...
        var jqxhr = $.getJSON(
          urlSocialInfoFor,
          { sl_type: t, sl_name: n, sl_value: v },
          function (j) {
    ...
    var socialFinalLoginResultsMt = function (j) {
      ...
      if (j.logged_in == "yes") {
        ...
        if ($("body").data("DynamicDetail") != undefined) {
          //We had to add this sine it is not always populated by the time this is run
    ...
    var onBtnClickSocialLoginChilli = function (a) {
      socialName = a;
      var urlStatus =
        location.protocol + "//" + uamIp + ":" + uamPort + "/json/status";
      $.ajax({
        url: urlStatus + "?callback=?",
        dataType: "jsonp",
        timeout: ajaxTimeout,
      })
        .done(function (j) {
          ...
          if (j.clientState == 1) {
            //FIXME Think we should redirect to Social Login Login...
            if (userName.startsWith("sl_")) {
              // Check if the logged in user is of the special social login type
              if (redirect_check) {
                // Check if the dynamic login page instructs us to redirect to a URL
                execRedirect(redirect_url);
              } else {
                coovaRefresh();
              }
            } else {
              // If the user is not of the social login type, simply refresh and show status page
              coovaRefresh();
            }
          }
        })
        .fail(function () {
          ...
          if (currentRetry <= retryCount) {
            ...
            fDebug("Timed out");
            fShowError(
              i18n("sLatest_Challenge_could_not_be_fetched_from_hotspot"),
            );
            loadingReset();
            currentRetry = 0;
          }
    ...
    var socialTempEncPwd = function (challenge) {
      $.ajax({
        ...
        dataType: "jsonp",
        timeout: ajaxTimeout,
        data: { challenge: challenge, password: password },
      })
        .done(function (j) {
    ...
    var socialTempLogin = function (encPwd) {
      var urlLogin =
        location.protocol + "//" + uamIp + ":" + uamPort + "/json/logon";
      $.ajax({
        url: urlLogin + "?callback=?",
        dataType: "jsonp",
        timeout: ajaxTimeout,
        data: { username: userName, password: encPwd },
      })
        .done(function (j) {
    ...
    var socialTempDisconnect = function () {
      var urlLogoff =
        location.protocol + "//" + uamIp + ":" + uamPort + "/json/logoff";
      $.ajax({
        url: urlLogoff + "?callback=?",
        dataType: "jsonp",
        timeout: ajaxTimeout,
      })
        .done(function (j) {
    ...
    var socialFinalLoginResults = function (j) {
      ...
      if (j.clientState == 0) {
        fShowError(
          extractHotspotError(
            j,
            i18n("sSocial_Login_user_failed_authentication"),
          ),
        );
      } else {
        if (redirect_check) {
    ...
    var onBtnClickToConnectClick = function (event) {
      ...
      var c_t_c_username =
        cDynamicData.settings.click_to_connect.connect_username +
        "@" +
        element_val;
      var c_t_c_password =
        cDynamicData.settings.click_to_connect.connect_username;
    ...
    var onBtnConnectClick = function (event) {
      //Get the latest challenge and continue from there onwards....
      event.preventDefault();
      ...
      if ($("#txtVoucher").length) {
        //It might not be there depending on the settings
        if ($("#txtVoucher").val() !== "") {
      ...
      $.ajax({
        ...
        .done(function (j) {
          ...
          if (j.clientState == 0) {
            ...
            //User Registration
            if (
              cDynamicData.settings.register_users == true &&
            ...
            //Lost Password
            if (
              cDynamicData.settings.lost_password == true &&
              cDynamicData.settings.user_login_check == true
            ) {
          ...
          if (j.clientState == 1) {
            ...
            if (cDynamicData.settings.redirect_check) {
              ...
              $("#hLogin").html(
                '<i class="bi-star"></i> ' + i18n("sConnected"),
              );
              if (counter == undefined) {
                //If it is the first time so initialise the loop counter
                sessionData = j;
                refreshCounter();
              }
            }
      ...
      $.ajax({
        ...
        .fail(function () {
          ...
          if (currentRetry <= retryCount) {
            ...
            fDebug("Timed out");
            fShowError(
              i18n("sThe_hotspot_is_not_responding_to_status_queries"),
            );
            loadingReset();
          }
        });
    ...
    var populateRegister = function () {
      ...
      var $form =
        `
             <div class="mb-3">
                <input type="text" placeholder="` +
        i18n("sFirstName") +
        `" class="form-control" id="txtrFirstName" name="name">
              </div>
              <div class="mb-3">
                <input type="text" placeholder="` +
        i18n("sSurname") +
        `" class="form-control" id="txtrSurname" name="surname">
              </div>
              <div class="mb-3 p-1 bg-secondary border">
                <input type="email" placeholder="` +
        i18n("sEmail_br_username_br") +
        `" class="form-control" id="txtrEmail" name="username" required>
                <div class="invalid-feedback">
                     ` +
        i18n("sPlease_supply_a_valid") +
        " " +
        ...
        `
              ...
              <div class="mb-3 p-1 bg-secondary border">
                <input type="text" placeholder="` +
        i18n("sPassword") +
        `" class="form-control" id="txtrPassword" name="password" minlength="5" required>
                <div class="invalid-feedback">
                    ` +
        ...
        `
              ...
              <div class="mb-3 p-1 bg-secondary border">
                <input type="text" placeholder="` +
        i18n("sCell") +
        `" class="form-control" id="txtrCell" name="phone" pattern="^[0-9]*$" minlength="8" required>
                <div class="invalid-feedback">
                    ` +
        i18n("sThe_number_must_have_at_least_8_digits") +
        `
      ...
      if (cDynamicData.settings.reg_ppsk) {
        ...
        $form = $form.concat(
          `<div class="mb-3 p-1 bg-secondary border">
                	<input type="text" placeholder="` +
            i18n("sWiFi_Preshared_Key") +
            `" class="form-control" id="txtrPPSK" name="PPSK" pattern="^[0-9]*$" minlength="8">
                	<div class="invalid-feedback">
          ...
          if (data.success) {
            ...
            if (data.errors !== undefined) {
              ...
              Object.keys(data.errors).forEach((key) => {
                ...
                msg =
                  msg + "<b>" + key + "</b>  " + data.errors[key] + "<br>\n";
              });
      ...
      var $form =
        `       
              <div class="mb-3 p-1 bg-secondary border">
                <input type="email" placeholder="` +
        i18n("sEmail") +
        `" class="form-control" id="txtlEmail" name="email" required>
                <div class="invalid-feedback">
                    ` +
        i18n("sPlease_supply_a_valid") +
        " " +
      ...
      var $form =
        `       
              <div class="mb-3 p-1 bg-secondary border">
                <input type="text" placeholder="` +
        i18n("sCell") +
        `" class="form-control" id="txtlPhone" name="phone" minlength="8" pattern="^[0-9]*$" required>
                <div class="invalid-feedback">
                    ` +
        i18n("sThe_number_must_have_at_least_8_digits") +
        `
